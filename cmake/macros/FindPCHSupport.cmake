...
FUNCTION(GENERATE_CXX_PCH_COMMAND TARGET_NAME INCLUDE_FLAGS IN PCH_SRC OUT)
  ...
  # Target-scoped compile definitions (e.g. _TRINITY_CORE_CONFIG / _TRINITY_REALM_CONFIG)
  # also affect the code and must be part of the PCH flags, otherwise GCC rejects
  # the generated .gch as invalid and the fallback include fails.
  GET_TARGET_PROPERTY(TARGET_COMPILE_DEFINITIONS ${TARGET_NAME} COMPILE_DEFINITIONS)
  IF(NOT TARGET_COMPILE_DEFINITIONS STREQUAL "TARGET_COMPILE_DEFINITIONS-NOTFOUND")
    FOREACH(ITEM ${TARGET_COMPILE_DEFINITIONS})
      LIST(APPEND DEFINITION_FLAGS "-D${ITEM} ")
    ENDFOREACH(ITEM)
  ENDIF()
...
FUNCTION(ADD_CXX_PCH_GCC TARGET_NAME PCH_HEADER PCH_SOURCE)
  # Force-include the PCH header in every translation unit. We deliberately do
  # NOT generate a .gch file: GCC rejects .gch files whenever any per-target
  # compile definition differs (e.g. _TRINITY_CORE_CONFIG / _TRINITY_REALM_CONFIG),
  # and the legacy generator would then fall back to an -include of a header that
  # does not exist in the build directory, producing a hard error.
  SET_TARGET_PROPERTIES(
    ${TARGET_NAME} PROPERTIES
    COMPILE_FLAGS "-include ${CMAKE_CURRENT_SOURCE_DIR}/${PCH_HEADER}"
  )
