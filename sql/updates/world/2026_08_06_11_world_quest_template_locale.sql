...
-- A database that has already been migrated no longer has `locales_quest`, and
...
SET @copy_quest_template_locale := IF(
    (SELECT COUNT(*) FROM information_schema.TABLES
      WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'locales_quest') > 0,
    "
INSERT IGNORE INTO `quest_template_locale`
...
",
    'DO 0');
PREPARE _copy_quest_template_locale FROM @copy_quest_template_locale;
EXECUTE _copy_quest_template_locale;
DEALLOCATE PREPARE _copy_quest_template_locale;
...
SET @copy_quest_offer_reward_locale := IF(
    (SELECT COUNT(*) FROM information_schema.TABLES
      WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'locales_quest') > 0,
    "
INSERT IGNORE INTO `quest_offer_reward_locale` (`ID`, `locale`, `RewardText`)
...
",
    'DO 0');
PREPARE _copy_quest_offer_reward_locale FROM @copy_quest_offer_reward_locale;
EXECUTE _copy_quest_offer_reward_locale;
DEALLOCATE PREPARE _copy_quest_offer_reward_locale;
...
-- reading a missing table aborts the rest of the file. Run this only when the
-- old table is actually there.
SET @copy_quest_request_items_locale := IF(
    (SELECT COUNT(*) FROM information_schema.TABLES
      WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'locales_quest') > 0,
    "
INSERT IGNORE INTO `quest_request_items_locale` (`ID`, `locale`, `CompletionText`)
...
",
    'DO 0');
PREPARE _copy_quest_request_items_locale FROM @copy_quest_request_items_locale;
EXECUTE _copy_quest_request_items_locale;
DEALLOCATE PREPARE _copy_quest_request_items_locale;
