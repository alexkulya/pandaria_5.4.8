	...
	*nearestRef = 0;
	...
	for (int i = 0; i < polyCount; ++i)
	{
		...
		if (posOverPoly)
		{
			...
			//d = d > 0 ? d*d : 0;			
            // Instead of clamping to 0, thus losing relativity data about polys being closer to point
            // than walkableClimb, let's just negate the number. This way such polys will still be
            // favored (negative number is still lower than any possible squared distance), but we
            // will be able to determine which one is the closest by comparing them as usual.
            d = d > 0 ? d*d : -d*d;
		}
		else
		{
