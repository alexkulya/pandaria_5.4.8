...
Corpse* SpellScript::GetHitCorpse()
{
    if (!IsInTargetHook())
    {
...
int32 SpellScript::GetDispelCount() const
{
    if (!IsInTargetHook())
    {
        TC_LOG_ERROR("scripts", "Script: `%s` Spell: `%u`: function SpellScript::GetDispelCount was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }
    return m_spell->m_dispelCount;
}
...
void SpellScript::PreventEffect(SpellEffIndex effIndex)
{
    m_hitPreventEffectMask |= 1 << effIndex;
    PreventDefaultEffect(effIndex);
}

void SpellScript::PreventDefaultEffect(SpellEffIndex effIndex)
{
    m_hitPreventDefaultEffectMask |= 1 << effIndex;
}

void SpellScript::PreventEffectForTarget(Unit* target, SpellEffIndex effIndex)
{
    if (m_currentScriptState != SPELL_SCRIPT_HOOK_EFFECT_LAUNCH_TARGET)
    {
        TC_LOG_ERROR("scripts", "Script: `%s` Spell: `%u`: function SpellScript::PreventEffectForTarget was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return;
    }

    for (auto&& it : m_spell->m_UniqueTargetInfo)
    {
        if (it.targetGUID == target->GetGUID())
        {
            it.effectMask &= ~(1 << effIndex);
            return;
        }
    }
    TC_LOG_ERROR("scripts", "Script: `%s` Spell: `%u`: function SpellScript::PreventEffectForTarget was called, but function has no effect becasue specified target not found!", m_scriptName->c_str(), m_scriptSpellId);
}

void SpellScript::RemoveUnitTarget(uint64 guid)
{
    if (m_currentScriptState != SPELL_SCRIPT_HOOK_OBJECT_AREA_TARGET_SELECT)
    {
        TC_LOG_ERROR("scripts", "Script: `%s` Spell: `%u`: SpellScript::RemoveUnitTarget you probably don't know what you are doing", m_scriptName->c_str(), m_scriptSpellId);
        return;
    }

    for (auto itr = m_spell->m_UniqueTargetInfo.begin(); itr != m_spell->m_UniqueTargetInfo.end(); ++itr)
    {
        if (itr->targetGUID == guid)
        {
            m_spell->m_UniqueTargetInfo.erase(itr);
            break;
        }
    }
}
...
void SpellScript::SetEffectValue(int32 damage)
{
...
Spell::UsedSpellMods const& SpellScript::GetAppliedSpellMods() const
{
    return m_spell->m_appliedMods;
}

void SpellScript::AddTriggerFlag(TriggerCastFlags flags)
{
    // FUCKING C++ ENUMS
    m_spell->_triggeredCastFlags = TriggerCastFlags(m_spell->_triggeredCastFlags | flags);
}

SpellMissInfo SpellScript::GetHitResult() const
{
    switch (m_currentScriptState)
    {
        case SPELL_SCRIPT_HOOK_EFFECT_HIT_TARGET:
        ...
        case SPELL_SCRIPT_HOOK_HIT:
        case SPELL_SCRIPT_HOOK_AFTER_HIT:
            return m_spell->m_missInfo;
    }
    TC_LOG_ERROR("scripts", "SpellScript::GetHitResult useless call");
...
AuraScript::EffectCalcTickAmountHandler::EffectCalcTickAmountHandler(AuraEffectCalcTickAmountFnType handler, uint32 effIndex, uint16 effName)
    : AuraScript::EffectBase(effIndex, effName), _handler(handler) { }

void AuraScript::EffectCalcTickAmountHandler::Call(AuraScript* auraScript, AuraEffect const* eff, int32& amount)
{
    (auraScript->*_handler)(eff, amount);
}
...
AuraScript::EffectAbsorbHealHandler::EffectAbsorbHealHandler(AuraEffectAbsorbHealFnType effectHandlerScript, uint8 effIndex)
    : AuraScript::EffectBase(effIndex, SPELL_AURA_SCHOOL_HEAL_ABSORB)
{
    _effectHandlerScript = effectHandlerScript;
}

void AuraScript::EffectAbsorbHealHandler::Call(AuraScript* auraScript, AuraEffect* aurEff, uint32& heal, uint32& absorb)
{
    (auraScript->*_effectHandlerScript)(aurEff, heal, absorb);
}
...
AuraScript::AuraDropModChargeHandler::AuraDropModChargeHandler(AuraDropModChargeFnType _pEffectHandlerScript)
{
    _HandlerScript = _pEffectHandlerScript;
}

void AuraScript::AuraDropModChargeHandler::Call(AuraScript* auraScript, Spell* bySpell)
{
    (auraScript->*_HandlerScript)(bySpell);
}
...
bool AuraScript::_IsDefaultActionPrevented()
{
    switch (m_currentScriptState)
    {
        ...
        case AURA_SCRIPT_HOOK_EFFECT_ABSORB_HEAL:
...
Unit* AuraScript::GetTarget() const
{
    switch (m_currentScriptState)
    {
        ...
        case AURA_SCRIPT_HOOK_EFFECT_CALC_TICK_AMOUNT:
...
void AuraScript::InitStackAmount(uint32 stacks)
{
    if (m_currentScriptState != SPELL_SCRIPT_STATE_LOADING)
    {
        TC_LOG_ERROR("scripts", "Script: `%s` Spell: `%u` AuraScript::InitStackAmount can't be called anywhere besides of Load method", m_scriptName->c_str(), m_scriptSpellId);
        return;
    }
    m_aura->m_stackAmount = stacks;
}
