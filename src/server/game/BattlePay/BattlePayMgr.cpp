...
void BattlePayMgr::LoadEntryLocalesFromDb()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT ID, Locale, Title, Description"
        " FROM battle_pay_entry_locale");

    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 Battle Pay store entries locale strings. DB table `battle_pay_entry_locale` is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (!HasEntryId(ID))
        {
            TC_LOG_ERROR("sql.sql", "Table `battle_pay_entry_locale` (Entry: %u) has locale strings for non-existing Battle Pay entry.", ID);
            continue;
        }
        ...
        if (locale == LOCALE_enUS)
			continue;
        ObjectMgr::AddLocaleString(Title, locale, data.Title);
        ObjectMgr::AddLocaleString(fields[3].GetString(), locale, data.Description);
...
bool BattlePayMgr::HasPointsBalance(WorldSession* session, uint64 points)
{
    if (Player* player = session->GetPlayer())
    {
        ...
        {
            player->SendBattlePayMessage(sObjectMgr->GetTrinityString(LANG_BATTLE_PAY_HAS_POINTS_BALANCE, session->GetSessionDbLocaleIndex()));
            return false;
        }
    }
    else
