...
{
    public:
        explicit PlayerAI(Player* player) : UnitAI(static_cast<Unit*>(player)), me(player), _selfSpec(player->GetTalentSpecialization()), _isSelfHealer(PlayerAI::IsPlayerHealer(player)), _isSelfRangedAttacker(PlayerAI::IsPlayerRangedAttacker(player)) { }
        ...
        uint16 GetSpec(Player const* who = nullptr) const { return (!who || who == me) ? _selfSpec : who->GetTalentSpecialization(); }
...
{
    public:
        SimpleCharmedPlayerAI(Player* player, bool dummy) : PlayerAI(player), _dummy(dummy), _castCheckTimer(500), _chaseCloser(false), _forceFacing(true) { }
    ...
    private:
        ...
        void HandleCasts(uint32 const diff);
        void HandleReturn();
        bool _dummy;
        uint32 _castCheckTimer;
        bool _chaseCloser;
        bool _forceFacing;
        float _previousFacingUpdate = 0.0f;
};
