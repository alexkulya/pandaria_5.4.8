...
struct HyjalGOSpawn
{
    HyjalGOSpawn(float x, float y, float z, float o, float rx, float ry, float rz, float rw) : Position(::Position { x, y, z, o }), Rotation(rx, ry, rz, rw) { }
    ::Position Position;
    G3D::Quat Rotation;
};

HyjalGOSpawn VeinPos[14] = // spawn points of the ancient gem veins
{
...
Position AllianceOverrunGargPos[5] = // gargoyle spawn points in the alliance overrun
{
...
HyjalGOSpawn AllianceFirePos[92] = // spawn points for the fire visuals (GO) in the alliance base
{
...
HyjalGOSpawn HordeFirePos[65] = // spawn points for the fire visuals (GO) in the horde base
{
...
void hyjalAI::SpawnVeins()
{
    if (Faction == 0)
    {
        ...
        for (uint8 i = 0; i<7; ++i)
        {
            GameObject* gem = me->SummonGameObject(GO_ANCIENT_VEIN, VeinPos[i].Position.GetPositionX(), VeinPos[i].Position.GetPositionY(), VeinPos[i].Position.GetPositionZ(), VeinPos[i].Position.GetOrientation(), VeinPos[i].Rotation, 0);
            if (gem)
            ...
            case JAINA:
                if (instance && instance->GetData(DATA_ALLIANCE_RETREAT))
                {
                    ...
                    for (uint8 i = 0; i < 92; ++i)//summon fires
                        me->SummonGameObject(GO_ROARING_FLAME, AllianceFirePos[i].Position.GetPositionX(), AllianceFirePos[i].Position.GetPositionY(), AllianceFirePos[i].Position.GetPositionZ(), AllianceFirePos[i].Position.GetOrientation(), AllianceFirePos[i].Rotation, 0);
            ...
            case THRALL: //thrall
                if (instance && instance->GetData(DATA_HORDE_RETREAT))
                {
                    ...
                    for (uint8 i = 0; i < 65; ++i)//summon fires
                        me->SummonGameObject(GO_ROARING_FLAME, HordeFirePos[i].Position.GetPositionX(), HordeFirePos[i].Position.GetPositionY(), HordeFirePos[i].Position.GetPositionZ(), HordeFirePos[i].Position.GetOrientation(), HordeFirePos[i].Rotation, 0);
...
void hyjalAI::DoOverrun(uint32 faction, const uint32 diff)
{
    ...
    switch (faction)
    {
        case 0://alliance
            ...
            {
                Creature* unit = me->SummonCreature(GARGOYLE, AllianceOverrunGargPos[i].GetPositionX(), AllianceOverrunGargPos[i].GetPositionY(), AllianceOverrunGargPos[i].GetPositionZ(), AllianceOverrunGargPos[i].GetOrientation(), TEMPSUMMON_MANUAL_DESPAWN, 2*60*1000);
                if (unit)
                {
                    unit->SetHomePosition(AllianceOverrunGargPos[i]);
                    CAST_AI(hyjal_trashAI, unit->AI())->faction = Faction;
                    CAST_AI(hyjal_trashAI, unit->AI())->IsOverrun = true;
                    CAST_AI(hyjal_trashAI, unit->AI())->OverrunType = i;
                    unit->setActive(true);
