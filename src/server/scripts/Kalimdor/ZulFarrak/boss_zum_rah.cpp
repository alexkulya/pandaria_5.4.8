...
public:
    ...
    struct boss_zum_rahAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_HEALING_WAVE, urand(12 * IN_MILLISECONDS, 18 * IN_MILLISECONDS));
            events.ScheduleEvent(EVENT_WARD_OF_ZUM_RAH, 7 * IN_MILLISECONDS);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_WARD_OF_ZUM_RAH:
                        ...
                        events.ScheduleEvent(EVENT_WARD_OF_ZUM_RAH, urand(15 * IN_MILLISECONDS, 25 * IN_MILLISECONDS));
                        break;
                    case EVENT_HEALING_WAVE:
                        if (HealthAbovePct(70))
                        {
                            events.RescheduleEvent(EVENT_HEALING_WAVE, 5 * IN_MILLISECONDS);
                            break;
                        }
