...
enum q28615
{
    NPC_WINTERFALL_SHAMAN = 7439,
};

// 91633 - Earthsurge
class spell_earthsurge : public SpellScript
{
    PrepareSpellScript(spell_earthsurge);

    void FilterTargets(std::list<WorldObject*>& targets)
    {
        targets.remove_if([=](WorldObject* obj) { return obj->GetEntry() != NPC_WINTERFALL_SHAMAN; });
    }

    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_earthsurge::FilterTargets, EFFECT_ALL, TARGET_UNIT_DEST_AREA_ENTRY);
    }
};
