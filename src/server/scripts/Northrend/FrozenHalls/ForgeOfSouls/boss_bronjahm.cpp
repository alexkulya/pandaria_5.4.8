                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SHADOW_BOLT:
                            if (!me->IsWithinMeleeRange(me->GetVictim()) || events.IsInPhase(PHASE_2))
            ...
            void MovementInform(uint32 type, uint32 pointId)
            {
                ...
                if (instance)
                {
                    if (TempSummon* summ = me->ToTempSummon())
                    {
                        ...
                        if (GUID_LOPART(BronjahmGUID) != pointId)
                            return;
    ...
    public:
        ...
        class spell_bronjahm_magic_bane_SpellScript : public SpellScript
        {
            ...
            void RecalculateDamage()
            {
                ...
                const int32 maxDamage = GetCaster()->GetMap()->GetSpawnMode() == DUNGEON_DIFFICULTY_HEROIC ? 15000 : 10000;
