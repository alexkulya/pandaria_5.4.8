...
public:
    ...
    struct boss_sartharionAI : public ScriptedAI
    {
        ...
        std::vector<uint64> hitedByWaves;
        ...
        void Reset() override
        {
            ...
            drakeCount = 0;
            me->GetMap()->SetWorldState(WORLDSTATE_TWILIGHT_ASSIST, 0);
            me->GetMap()->SetWorldState(WORLDSTATE_TWILIGHT_DUO, 0);
            me->GetMap()->SetWorldState(WORLDSTATE_TWILIGHT_ZONE, 0);
            ...
            if (instance)
            {
                ...
                instance->SetData(DATA_LOOT_AVAILABLE, 0);
            }
        }
        ...
        bool WasAffectedByVolcano(uint64 guid)
        {
        ...
        void FetchDragons()
        {
            ...
            if (drakeCount >= 1)
            {
                me->GetMap()->SetWorldState(WORLDSTATE_TWILIGHT_ASSIST, 1);
                me->AddLootMode(LOOT_MODE_HARD_MODE_1);
            }

            if (drakeCount >= 2)
            {
                me->GetMap()->SetWorldState(WORLDSTATE_TWILIGHT_DUO, 1);
                me->AddLootMode(LOOT_MODE_HARD_MODE_2);
            }

            if (drakeCount == 3)
            {
                me->GetMap()->SetWorldState(WORLDSTATE_TWILIGHT_ZONE, 1);
                me->AddLootMode(LOOT_MODE_HARD_MODE_3); // HARD_MODE reference to our 10/25 difficulty mode - it doesn`t work currently

                if (instance)
                    instance->SetData(DATA_LOOT_AVAILABLE, DONE);
            }
        }
...
// Flame Tsunami 57491
class spell_sartharion_flame_tsunami : public SpellScript
{
    PrepareSpellScript(spell_sartharion_flame_tsunami);
    ...
    {
        OnEffectHitTarget += SpellEffectFn(spell_sartharion_flame_tsunami::HandleHitByWave, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};

typedef boss_sartharion::boss_sartharionAI SartharionAI;

class achievement_gonna_go_when_the_volcano_blows : public AchievementCriteriaScript
{
    public:
        achievement_gonna_go_when_the_volcano_blows() : AchievementCriteriaScript("achievement_gonna_go_when_the_volcano_blows") { }
        ...
        bool OnCheck(Player* source, Unit* target) override
        {
            ...
            if (SartharionAI* sartharionAI = CAST_AI(SartharionAI, target->GetAI()))
                return !sartharionAI->WasAffectedByVolcano(source->GetGUID());
