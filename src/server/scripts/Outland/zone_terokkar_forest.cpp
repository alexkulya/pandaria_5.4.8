...
struct npc_unkor_the_ruthless : public ScriptedAI
{
    npc_unkor_the_ruthless(Creature* creature) : ScriptedAI(creature) { }
    ...
    void DamageTaken(Unit* done_by, uint32 &damage) override
    {
        ...
        if (player && (me->HealthBelowPctDamaged(30, damage) || me->GetHealth() <= damage) && !hasTriggered)
        {
            if (Group* group = player->GetGroup())
            {
                for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
                {
                    ...
                    if (groupie &&
                        ...
                        groupie->GetReqKillOrCastCurrentCount(QUEST_DONTKILLTHEFATONE, 18260) == 10*/) // doesn`t work properly
                    {
                        hasTriggered = true;
                        damage = 0;
                ...
                player->GetReqKillOrCastCurrentCount(QUEST_DONTKILLTHEFATONE, 18260) == 10*/) // doesn`t work properly
            {
                hasTriggered = true;
                damage = 0;
