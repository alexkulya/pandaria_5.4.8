...
// 87649 - Satisfied (Achievement 5779 - You'll Feel Right as Rain)
class spell_achievement_youll_feel_right_as_rain : public AuraScript
{
    PrepareAuraScript(spell_achievement_youll_feel_right_as_rain);

    void HandleApply(AuraEffect const*, AuraEffectHandleModes)
    {
        // Gain 91 stacks of the Satisfied buff by eating Chocolate Cookies.
        if (GetStackAmount() >= 91)
            if (Player* player = GetUnitOwner()->ToPlayer())
                player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 99041);
    }

    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(spell_achievement_youll_feel_right_as_rain::HandleApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAPPLY);
    }
};
