...
struct boss_ordos : public ScriptedAI
{
    boss_ordos(Creature* creature) : ScriptedAI(creature), summons(me) { }
    ...
    void EnterCombat(Unit* who) override
    {
        ...
        events.ScheduleEvent(EVENT_ORDOS_MAGMA_CRUSH, 9s + 500ms);
        events.ScheduleEvent(EVENT_ORDOS_POOL_OF_FIRE, 27s);
        events.ScheduleEvent(EVENT_ORDOS_BURNING_SOUL, 20s + 500ms);
        events.ScheduleEvent(EVENT_ORDOS_ETERNAL_AGONY, 5min);
        events.ScheduleEvent(EVENT_ORDOS_ANCIENT_FLAME, 40s);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (events.ExecuteEvent())
        {
            case EVENT_ORDOS_MAGMA_CRUSH:
            {
                ...
                events.ScheduleEvent(EVENT_ORDOS_MAGMA_CRUSH, 15s);
                break;
            }
            case EVENT_ORDOS_ANCIENT_FLAME:
            {
                ...
                events.ScheduleEvent(EVENT_ORDOS_ANCIENT_FLAME, 42s);
                break;
            }
            case EVENT_ORDOS_BURNING_SOUL:
            {
                ...
                events.ScheduleEvent(EVENT_ORDOS_BURNING_SOUL, 29s);
                break;
            }
            ...
            case EVENT_ORDOS_POOL_OF_FIRE:
            {
                ...
                events.ScheduleEvent(EVENT_ORDOS_POOL_OF_FIRE, 31s);
                break;
            }
            default:
                break;
...
{
    register_creature_script(boss_ordos);
    register_creature_script(npc_ordos_ancient_flame);
    register_spell_script(spell_ordos_magma_crush);
    register_aura_script(spell_ordos_burning_soul);
