...
struct boss_niuzao : public ScriptedAI
{
    boss_niuzao(Creature* creature) : ScriptedAI(creature), summons(me) 
    {
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        ...
        events.ScheduleEvent(EVENT_NIUZAO_OXEN_FORTITUDE, 25s);
        events.ScheduleEvent(EVENT_NIUZAO_MASSIVE_QUAKE, 30s);
        events.ScheduleEvent(EVENT_NIUZAO_HEADBUTT, 12s);
    ...
    void DamageTaken(Unit* attacker, uint32& damage) override
    {
        ...
        {
            nextCharge -= 33.0f;
            events.ScheduleEvent(EVENT_NIUZAO_CHARGE_PREPARE, 500ms);
        }
        ...
        if (damage >= me->GetHealth() && !death)
        {
            ...
            events.Reset();
            summons.DespawnAll();
            nonCombatEvents.ScheduleEvent(EVENT_NIUZAO_SHAO_DO_OUTRO, 10s);
            nonCombatEvents.ScheduleEvent(EVENT_NIUZAO_DEATH, 13s);
            death = true;
        ...
        switch (pointId)
        {
            ...
            case MOVEMENT_NIUZAO_CHARGE:
            {
                if (_charges >= 4)
                {
                    ...
                    events.Reset();
                    events.ScheduleEvent(EVENT_NIUZAO_OXEN_FORTITUDE, 12s);
                    events.ScheduleEvent(EVENT_NIUZAO_MASSIVE_QUAKE, 15s);
                    events.ScheduleEvent(EVENT_NIUZAO_HEADBUTT, 6s);
                    return;
                }
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_NIUZAO_MASSIVE_QUAKE:
                {
                    ...
                    Talk(SAY_SPELL_3);
                    events.ScheduleEvent(EVENT_NIUZAO_MASSIVE_QUAKE, 35s);
                    break;
                }
                case EVENT_NIUZAO_HEADBUTT:
                {
                    ...
                    events.ScheduleEvent(EVENT_NIUZAO_HEADBUTT, 20s + 500ms);
                    break;
                }
                case EVENT_NIUZAO_OXEN_FORTITUDE:
                {
                    ...
                    events.ScheduleEvent(EVENT_NIUZAO_OXEN_FORTITUDE, 30s);
                    break;
                }
...
{
    register_creature_script(boss_niuzao);
    register_spell_script(spell_niuzao_headbutt);
    register_aura_script(spell_niuzao_charge);
}
